...
function computeVectors(elements) {
  ...
  elements.forEach(element => {
    switch (element.encoding) {
      case 'bits':
        a = a.concat(utils.hexToBin(utils.strip0x(element.value)));
        break;
      case 'bytes':
        a = a.concat(utils.hexToBytes(utils.strip0x(element.value)));
        break;
      case 'field': // each vector element will be a 'decimal representation' of integers modulo a prime. p=21888242871839275222246405745257275088548364400416034343698204186575808495617 (roughly = 2*10e76 or = 2^254)
        a = a.concat(
          utils.hexToFieldPreserve(
            utils.strip0x(element.value),
            config.ZOKRATES_PACKING_SIZE,
            element.packets,
          ),
        );
        break;
      case 'point': // x,y point on an elliptic curve. Note - these are BigInts not hex strings
        a = a.concat(element.value[0].toString(10)).concat(element.value[1].toString(10));
        break;
      case 'scalar': // scalar to multiply a point on an elliptic curve. Note - these are BigInts not hex strings but they need to be encoded as packed fields because scalarMult works on binary bits for the scalar
        a = a.concat(
          utils.hexToFieldPreserve(
            utils.strip0x(element.value.toString(16)),
            config.ZOKRATES_PACKING_SIZE,
            element.packets,
          ),
        );
