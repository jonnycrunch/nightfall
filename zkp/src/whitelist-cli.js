/* globals BigInt */
/**
module to provide a cli for whitelist manipulation - remove once UI is working
*/
...
import { dec2, scalarMult, babyjubjub } from './encrypted-commitment';
...
const options = yargs
  ...
  .option('whitelisted', { describe: 'is the address whitelisted?', type: 'string' })
  .option('enable', { describe: 'enable the whitelist', type: 'boolean' })
  .option('disable', { describe: 'disable the whitelist', type: 'boolean' })
  .option('add', { describe: 'add address to whitelist', type: 'string' })
  .option('list', { describe: 'list accounts on this node', type: 'boolean' })
  .option('remove', { describe: 'remove address from whitelist', type: 'string' }).argv;

function bruteForce(m1) {
  let i = 0;
  let p = [];
  do {
    p = scalarMult(i, points[1]);
    i++;
...
web3.eth.getAccounts().then(accounts => {
  ...
  FTokenShield.deployed().then(shield => {
    if (options.add) shield.addToWhitelist(options.add, { from: accounts[0] });
    if (options.remove) shield.removeFromWhitelist(options.remove, { from: accounts[0] });
    if (options.whitelisted)
      shield.isWhitelisted.call(options.whitelisted).then(answer => console.log(answer));
    if (options.enable) shield.enableWhitelist({ from: accounts[0] });
    if (options.disable) shield.disableWhitelist({ from: accounts[0] });
    if (options.decrypt) {
